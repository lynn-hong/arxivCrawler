...
class CrawlerFrame():
    ...
    def startWork2(self, year):
        print("\n%s" % str(datetime.now()))
        self.currentYear = re.search(r"20(\d+)", str(year)).group(1)
        print("Get file name list finished...")
        moreIdSet = self.getIdFromFile(self.resultPath.replace("pdf_cs/%s/", "")+"more_2014.txt")
        print("%i ids..." % len(moreIdSet))
        idx = 0
        for p_id in moreIdSet:
    ...
    def apiRequests(self, year):
        ...
        self.currentYear = re.search(r"20(\d+)", str(year)).group(1)
        #existingFileSet = self.getFileNames(self.resultPath % ("20%s" % self.currentYear))
        requestIdSet = self.getIdFromFile(self.resultPath.replace("pdf_cs/%s/", "")+"arxiv_id_2013.tsv")
        print("Get file name list finished...")
        '''sql = "SELECT p_id FROM paper WHERE p_id LIKE 'cs\_%s%%' OR p_id LIKE '%s%%';" \
        ...
        for p_id in requestIdSet:
            ...
            if idx % 100 == 0:
                ...
                print("%s\n" % str(datetime.now()))
            err_code, r = self.r.access_page(self.url_api_desc % p_id.replace("_", "/"), 4, pdf=True)
            if err_code != 1:
